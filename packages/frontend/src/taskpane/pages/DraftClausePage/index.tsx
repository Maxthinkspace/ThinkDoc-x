...
import { ArrowLeft, FileText, Book, Folder, ChevronDown, Check, Loader, Sparkles } from "lucide-react";
import { useLanguage } from "../../contexts/LanguageContext";
import { useToast } from "../../hooks/use-toast";
import { useNavigation } from "../../hooks/use-navigation";
import { backendApi } from "../../../services/api";
import { buildApiUrl } from "../../../services/apiBaseUrl";
import "./DraftClausePage.css";
...
export const DraftClausePage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    fetchPlaybooks();
    fetchProjects();
  }, [fetchClauses, fetchPlaybooks, fetchProjects]);
  ...
  const handleDraft = async () => {
    ...
    try {
      ...
      // Insert drafted text into Word document at cursor position
      try {
        await Word.run(async (context) => {
          const selection = context.document.getSelection();
          selection.insertText(draftedText + "\n", "Replace");
          await context.sync();
        });
      } catch (wordError) {
        console.error("Failed to insert drafted clause:", wordError);
        toast({
          title: "Clause Drafted",
          description: "Clause drafted successfully, but failed to insert into document. Please copy manually.",
          variant: "warning",
        });
        return;
      }
      ...
      toast({
        title: "Clause Drafted",
        description: "Your clause has been drafted and inserted into the document.",
      });
  ...
  return (
    <div className="draft-clause-page">
      {/* Header */}
      <div className="draft-clause-header">
        <button className="draft-clause-back-button" onClick={() => navigateTo("dashboard")}>
          <ArrowLeft size={20} />
        </button>
        <div className="draft-clause-header-content">
          <h1 className="draft-clause-title">Draft Clause</h1>
          <p className="draft-clause-subtitle">Create a new clause using AI assistance</p>
        </div>
      </div>
