...
interface RedomicileProgressPageProps {
  jobId: string;
  progress?: {
    currentStep: number;
    totalSteps: number;
...
export const RedomicileProgressPage: React.FC<RedomicileProgressPageProps> = ({
  jobId,
  ...
  useEffect(() => {
    if (pollingResult.status === "done" && pollingResult.result) {
      ...
      if (result && result.success !== false) {
        // Fetch context from jobTracker
        const trackedJob = jobTracker.getJobByJobId(jobId);
        const originalParsed = trackedJob?.inputContext?.data?.originalParsed || null;
        const config = trackedJob?.inputContext?.data?.config || null;
        
        // Extract sections and metadata from result
        const sections = result.sections || [];
        const metadata = result.metadata || null;
        
        if (config) {
          onComplete(sections, metadata, originalParsed, config);
        }
      }
  ...
  return (
    <div className={styles.pageRoot}>
      ...
      <div className={styles.header}>
        ...
        <h1 className={styles.title}>Redomicile Agreement</h1>
      </div>

      {/* Content */}
      <div className={styles.content}>
        <div className={styles.progressCard}>
          <h3 className={styles.progressTitle}>Processing Redomicile</h3>
